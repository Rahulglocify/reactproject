...
function UpdateProduct(props) {
  ...
  useEffect(async () => {
    let result = await fetch(
      "http://127.0.0.1:8000/api/product-details/" + productid
    );
  ...
  return (
    <div>
      ...
      <div className="col-sm-6 offest-sm-3">
        ...
        <br />
        <img
          src={"http://127.0.0.1:8000/uploads/product/" + data.image}
          style={{ width: 100 }}
        />
        <button className="btn btn-primary" onClick={productUpdate}>
          Update Product
        </button>
      </div>
    </div>
